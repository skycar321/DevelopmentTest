# Vue 3 draft kit — 2026-09-09 (encrypted, sixth cut: lab tip f591bd4e)

This directory publishes one archive, `vue3-draft-kit-20260909-6.7z`.
...
```bash
shasum -a 256 -c sha256.txt        # or: certutil -hashfile vue3-draft-kit-20260909-6.7z SHA256
7z x -p vue3-draft-kit-20260909-6.7z # enter the password when prompted
```
...
Sixth cut of the same day; supersedes the fifth. Skill bundle unchanged (lab tip f591bd4e). The HTML copy of the in-house runbook in `docs/` is now a self-contained offline document (proper document head, no external font or script references, system fonts) so it opens correctly on a closed-network machine; the runbook refers to "the newest `vue3-draft-kit-*` directory" instead of a cut number. Markdown runbook and the to-be manifest are the same as in the fifth cut.

...
## 조회가 안 될 때 — 폐쇄망에서 가장 흔한 경우

결과 파일에 `CRYPT_E_NO_REVOCATION_CHECK` 가 보이면 **인증서 체인은 이미 신뢰되고 있습니다.**
폐쇄망이라 인증서 폐지목록(CRL/OCSP)으로 나갈 수 없어 검사가 끝나지 못한 것뿐입니다.
CA 번들 문제가 아니므로 `-CaBundle` 은 도움이 되지 않습니다.

```powershell
... -SslNoRevoke      # 폐지검사만 생략, 체인 검증은 그대로 유지
```

이 도구는 해당 오류를 만나면 **자동으로 1회 재시도**하고 그 사실을 결과 파일에 남깁니다.

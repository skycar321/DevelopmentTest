# Vue 3 draft kit — 2026-09-11 (31st)

This directory holds one encrypted archive (`vue3-draft-kit-20260911-31.7z`, 7z AES-256 with
...
7z x -p<password> vue3-draft-kit-20260911-31.7z
```
...
## Printable explainer for screen owners (new in this kit)

`docs/옵션스-컴포지션-학습카드.html` opens in any browser with no external library or font. It shows
how Options API and Composition API lay out the *same* feature, and answers the question screen
owners ask first: whether their own screen has to be rewritten. It does not — the shared layer was
built so it doesn't have to. Eleven shared components use `<script setup lang="ts">`, but all
fifteen composables deliberately avoid `ref`/`computed`/`onMounted`, so they stay callable from
`methods` in an untouched Options API screen. Hand this out with the assignment notes.

## Meeting follow-up (new in this kit)

`docs/회의-회신-메일-20260911.txt` is the reply that went out, and
`docs/회의-논의사항-상세-20260911.md` records the ten items raised in the 2026-09-11 review and
what the current artifacts actually show against each: the dependency-quarantine comparison and
what it does not yet cover, the 75 library decisions split by disposition, where the shared-component
selection is still unproven, and why a reduced first pass still has to carry the library
replacements. The emailed reply is a summary; the figures and the reasoning live here.
